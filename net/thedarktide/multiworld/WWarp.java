...
public class WWarp
{
	...
	public WWarp(Player p, String[] args, Configuration Worlds, Configuration Config)
	{
		...
		if (p.getServer().getWorld(args[0]) != null)
		{
			...
			p.teleport(loc);
			p.sendMessage(ChatColor.DARK_RED + "[DarkPortals]: " + ChatColor.GREEN + "Welcome to " + args[0]);
			return;
		}
		p.sendMessage(ChatColor.DARK_RED + "[DarkPortals]: " + ChatColor.RED + "That world does not exist.");
		p.sendMessage(ChatColor.DARK_RED + "[DarkPortals]: " + ChatColor.RED + "Use /wlist to see worlds.");
	}
