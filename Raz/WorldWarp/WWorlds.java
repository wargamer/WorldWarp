...
public class WWorlds {
	...
	public WWorlds(Player p, String[] args, Configuration Worlds,
			Configuration Config) {
		Worlds.load();
		if (args.length < 2) {
			p.sendMessage(ChatColor.DARK_RED + "[WorldWarp]: " + ChatColor.RED
					+ "Use /wcreate [name] [environmate] <seed>.");
			...
			World.Environment e = World.Environment.valueOf(args[1]
					.toUpperCase());
			p.sendMessage(ChatColor.DARK_RED + "[WorldWarp]: " + ChatColor.RED
			...
			Worlds.setProperty("worlds." + args[0] + ".environmate",
					args[1].toUpperCase());
